...
const GL_LIGHT1   = 1003;
const GL_LIGHT2   = 1004;
const GL_POSITION = 1005;
const GL_DIFFUSE  = 1006;
const GL_SPECULAR = 1007;
const GL_AMBIENT  = 1008;
...
function glInit() {
    ...
    _GL.materialSpecular  =     0.75;
    _GL.materialShininess =     8;
    ...
    _GL.light0Ambient  = [1.0, 1.0, 1.0, 1.0];
    _GL.light0Diffuse  = [1.0, 1.0, 1.0, 1.0];
    _GL.light0Specular = [1.0, 1.0, 1.0, 1.0];
    _GL.light1         = false;
    _GL.light1Position = [0.0, 0.0, 0.0, 1.0];
    _GL.light1Ambient  = [0.0, 0.0, 0.0, 1.0];
    _GL.light1Diffuse  = [0.0, 0.0, 0.0, 1.0];
    _GL.light1Specular = [0.0, 0.0, 0.0, 1.0];
    _GL.light2         = false;
    _GL.light2Position = [0.0, 0.0, 1.0, 1.0];
    _GL.light2Ambient  = [1.0, 1.0, 1.0, 1.0];
    _GL.light2Diffuse  = [1.0, 1.0, 1.0, 1.0];
    _GL.light2Specular = [1.0, 1.0, 1.0, 1.0];
    //
...
function _GLinitMaterialShader(shdrInfo) {
    ...
    //
    shdrInfo.uLight0Ambient =
        gl.getUniformLocation(shdrPrgm,'uLight0Ambient');
    shdrInfo.uLight0Diffuse =
        gl.getUniformLocation(shdrPrgm,'uLight0Diffuse');
    shdrInfo.uLight0Specular =
        gl.getUniformLocation(shdrPrgm,'uLight0Specular');
    ...
    //
    shdrInfo.uLight1Ambient =
        gl.getUniformLocation(shdrPrgm,'uLight1Ambient');
    shdrInfo.uLight1Diffuse =
        gl.getUniformLocation(shdrPrgm,'uLight1Diffuse');
    shdrInfo.uLight1Specular =
        gl.getUniformLocation(shdrPrgm,'uLight1Specular');
    shdrInfo.uLight1Position =
        gl.getUniformLocation(shdrPrgm,'uLight1Position');
    shdrInfo.uLight1Enabled =
        gl.getUniformLocation(shdrPrgm,'uLight1Enabled');
    //
    shdrInfo.uLight2Ambient =
        gl.getUniformLocation(shdrPrgm,'uLight2Ambient');
    shdrInfo.uLight2Diffuse =
        gl.getUniformLocation(shdrPrgm,'uLight2Diffuse');
    shdrInfo.uLight2Specular =
        gl.getUniformLocation(shdrPrgm,'uLight2Specular');
    shdrInfo.uLight2Position =
        gl.getUniformLocation(shdrPrgm,'uLight2Position');
    shdrInfo.uLight2Enabled =
        gl.getUniformLocation(shdrPrgm,'uLight2Enabled');
    //
...
function glBegin(what, name, saveColors = false, computeNormals = false) {
...
function glEnd() {
    ...
    if (_GL.computeNormals) {
        console.assert(_GL.recording.mode == GL_TRIANGLES);
        _GL.normals = [];

        for (let i = 0; i < _GL.points.length; i += 12) {
            const v0 = [_GL.points[i+0],_GL.points[i+1],_GL.points[i+2]];
            const v1 = [_GL.points[i+4],_GL.points[i+5],_GL.points[i+6]];
            const v2 = [_GL.points[i+8],_GL.points[i+9],_GL.points[i+10]];
            const p0 = new Point3d(v0[0],v0[1],v0[2]);
            const p1 = new Point3d(v1[0],v1[1],v1[2]);
            const p2 = new Point3d(v2[0],v2[1],v2[2]);
            const u = p1.minus(p0);
            const v = p2.minus(p0);
            const w = u.cross(v);
            // const w = u.cross(v).neg();
            const n = w.unit();
            _GL.normals.push(n.dx);
            _GL.normals.push(n.dy);
            _GL.normals.push(n.dz);
            _GL.normals.push(0.0);
            _GL.normals.push(n.dx);
            _GL.normals.push(n.dy);
            _GL.normals.push(n.dz);
            _GL.normals.push(0.0);
...
function _glRenderMaterial(shader, recording) {
    ...
    //
    //
    gl.uniform4fv(shader.uLight0Ambient,  _GL.light0Ambient);
    gl.uniform4fv(shader.uLight0Diffuse,  _GL.light0Diffuse);
    gl.uniform4fv(shader.uLight0Specular,  _GL.light0Specular);
    ...
    gl.uniform4fv(shader.uLight1Ambient,  _GL.light1Ambient);
    gl.uniform4fv(shader.uLight1Diffuse,  _GL.light1Diffuse);
    gl.uniform4fv(shader.uLight1Specular,  _GL.light1Specular);
    gl.uniform4fv(shader.uLight1Position, _GL.light1Position);
    if (_GL.light1) {
        gl.uniform1i(shader.uLight1Enabled, 1);
    } else {
        gl.uniform1i(shader.uLight1Enabled, 0);
    }
    gl.uniform4fv(shader.uLight2Ambient,  _GL.light2Ambient);
    gl.uniform4fv(shader.uLight2Diffuse,  _GL.light2Diffuse);
    gl.uniform4fv(shader.uLight2Specular,  _GL.light2Specular);
    gl.uniform4fv(shader.uLight2Position, _GL.light2Position);
    if (_GL.light2) {
        gl.uniform1i(shader.uLight2Enabled, 1);
    } else {
        gl.uniform1i(shader.uLight2Enabled, 0);
    }
    //
    // ...material info.
...
function glLightfv(which, what, values) {
    let vs = values.concat([]);
    if (values.length == 3) {
        vs = values.concat([1.0]);
    }
    if (what == GL_POSITION) {
        if (which == GL_LIGHT0) {
            _GL.light0Position = vs;
        } else if (which == GL_LIGHT1) {
            _GL.light1Position = vs;
        } else if (which == GL_LIGHT2) {
            _GL.light2Position = vs;
        }
    } else if (what == GL_DIFFUSE) {
        if (which == GL_LIGHT0) {
            _GL.light0Diffuse = vs;
        } else if (which == GL_LIGHT1) {
            _GL.light1Diffuse = vs;
        } else if (which == GL_LIGHT2) {
            _GL.light2Diffuse = vs;
        }
    } else if (what == GL_SPECULAR) {
        if (which == GL_LIGHT0) {
            _GL.light0Specular = vs;
        } else if (which == GL_LIGHT1) {
            _GL.light1Specular = vs;
        } else if (which == GL_LIGHT2) {
            _GL.light2Specular = vs;
        }
    } else if (what == GL_AMBIENT) {
        if (which == GL_LIGHT0) {
            _GL.light0Ambient = vs;
        } else if (which == GL_LIGHT1) {
            _GL.light1Ambient = vs;
        } else if (which == GL_LIGHT2) {
            _GL.light2Ambient = vs;
        }
    }
}
