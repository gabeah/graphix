...
class Surface {
    ...
    clone_pos(P) {
        ...
        let source_edge = P.edge;
        let crawler = source_edge;
    ...
    subdivide() {
        ...
        for (let f of S.allFaces()) {
            ...
            const vi0_pos = R.clone_pos(f.edge.target)
            const vi1_pos = R.clone_pos(f.edge.next.target)
            const vi2_pos = R.clone_pos(f.edge.prev.target)

            const vi0 = R.makeVertex(vi0_pos);
            const vi1 = R.makeVertex(vi1_pos);
            const vi2 = R.makeVertex(vi2_pos);

            console.log("Finding clone positions")
            R.clone_pos(f.edge.target);

            console.log("Assigning Clones")
            f.edge.target.clone = vi0;
            f.edge.next.target.clone = vi1;
            f.edge.prev.target.clone = vi2;

            console.log("Finding split locations")
            ...
            console.log("Making split vertices")
            ...
            console.log("Assigning splits")
            if (f.edge.split == null) {
                f.edge.split = split01;
                f.edge.twin.split = split01;
            }
            if (f.edge.next.split == null) {
                f.edge.next.split = split12;
                f.edge.next.twin.split = split12;
            }
            if (f.edge.prev.split == null) {
                f.edge.prev.split = split20;
                f.edge.prev.twin.split = split20;
            }
